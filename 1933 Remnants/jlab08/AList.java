...
*/
    ...
    public AList<T> slice(int start, int stop, int step) {
        AList<T> l = new AList<T>((stop - start) / step);
        while(start <= stop) {
            l.add(this.getEntry(start));
            start += step;
        }
        return l;
    }
    public void sort(boolean ascending) {
        for(int i = 1; i <= numberOfEntries; i++) {
            int min = i;
            for(int o = i + 1; o <= numberOfEntries; o++) {
                if(((ascending ? 1 : -1) * getEntry(o).compareTo(getEntry(min))) < 0) {
                    min = o;
                }
            }
            T temp = getEntry(min);
            replace(min, getEntry(i));
            replace(i, temp);
        }
    }
    ...
    public static AList<String> fileToAList(File input) {
        ...
        int c = 0;
        while(s.hasNextLine()) {
            String[] spls = s.nextLine().split(", ", 2);
            map.put(Integer.parseInt(spls[0]), spls[1]);
            c++;
        }
        AList<String> n = new AList<String>();
        for(int i = 0; i < c; i++) {
            n.add(map.get(i));
        }
        s.close();
