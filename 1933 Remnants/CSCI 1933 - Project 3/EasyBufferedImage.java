 ...
 * of the built-in image classes. Performance (in terms of memory and speed) is
 * sacrificed in order to provide
 * simplified access to pixel data. Allows direct read/write access to
 * individual pixels
 * and a factory method for creating EasyBufferedImages from files. The image
 * always
 * appears as either a gray-scale image, an RGB image or an RGBA image. The data
 * is
 * never packed or interleaved (at least with respect to the get/set pixel
 * methods).
 * <p>
 * Please note that grayscale processing of a <b>color</b> image always
 * maintains the color information of the image while
 * altering the <b>intensity</b> channel. To perform operations on a purely
 * gray-scale image, convert any color
 * images to grayscale prior to processing.
 * <p>
 * Since the EasyBufferedImage class is a sub-class of Image and BufferedImage,
 * it
 * is fully integrated into the standard Swing libraries for viewing and
 * manipulation.
 ...
 * 
 * </blockquote></b>
 * where the filename is a String representing an image file in either GIF, PNG,
 * or JPEG
 * formats. The following code fragment is an example of how to invert a
 * gray-scale image:
 * <b><font color="#115500"><blockquote>
 * 
 * <pre>
...
public class EasyBufferedImage extends BufferedImage {
	 ...
	 * The dimensions of the pixel array must be [height][width][bands] where bands
	 * may
	 * be either 1 (gray-scale) or 3 (red-green-blue).
	 * This is a factory method to create EasyBufferedImages. There is no direct
	 * constructor access.
	 * <p>
	 * 
	 * @param pixels is an array of [height][width][bands] pixels that represents
	 *               the image.
	 ...
	 * <p>
	 * 
	 * @param pixels is an array of [height][width] pixels that represents the
	 *               image.
	 * @return An EasyBufferedImage representation of the pixels.
	 * @throws IllegalArgumentException if pixels is null
	 * @see Image
	 ...
	 * @return An EasyBufferedImage representation of the pixels.
	 * @throws IllegalArgumentException if pixels is null or the length is not width
	 *                                  * height.
	 * @see Image
	 * @see BufferedImage
	 */
	 ...
	 * order) RED (or GRAY), GREEN, BLUE, ALPHA. Pixel values are in the range
	 * 0-255.
	 * <p>
	 * 
	 * @param None
	 ...
	 * EasyBufferedImage.
	 * The number of bands will be 1 for a gray-scale or binary image, 3 for a color
	 * image without
	 * transparency, and 4 for a color image with transparency. The bands are (in
	 * order) RED (or GRAY), GREEN, BLUE, ALPHA. Pixel values are in the range
	 * 0-255.
	 ...
	 * The array is layed out in row-major format and contains height*width pixel
	 * values.
	 * The number of bands will be 1 for a gray-scale or binary image, 3 for a color
	 * image without
	 * transparency, and 4 for a color image with transparency. The bands are (in
	...
	public int[] getPixels1D() {
		...
		for (int i = 0; i < height; i++) {
			for (int j = 0; j < width; j++) {
				for (int k = 0; k < bands; k++) {
					pixels[i * width * bands + j * bands + k] = raster.getSample(j, i, k); // probably not correct
																							// here.............
				}
			}
		}
	...
	/**
	 * Sets all of the EasyBufferedImages pixels in the specified band. The number
	 * of elements
	 * in the array must be equal to height*width of the EasyBufferedImage. The band
	 * must
	 * be either GRAY, RED, GREEN, BLUE, or ALPHA. Pixel values not in the range
	 * 0-255
	 ...
	 * @throws IllegalArgumentException if the pixels array is not compatible with
	 *                                  the image.
	 * @return None
	 */
	...
	/**
	 * Sets all of the EasyBufferedImages pixels in the specified band. The
	 ...
	 * must correspond to [height][width][bands] of the EasyBufferedImage. The
	 * number of bands
	 * will be 1 for a gray-scale or binary image, 3 for a color image without
	 * transparency, and 4 for a color image with transparency. The bands are (in
	 ...
	 * @param format   one of EasyBufferedImage.PNG, EasyBufferedImage.GIF or
	 *                 EasyBufferedImage.JPEG
	 * @return None
	...
	/**
	 * A demo application that shows simple ways of using the EasyBufferedImage
	 * class. Execute from the command line
	 * by typing
	 * <p>
	 * <b><blockquote>
	 * 
	 ...
	 * 
	 * </blockquote></b>
	 * where the <tt>AlphaValue</tt> is a percentage value in the range [0..100] and
	 * serves as a brightness control. Values in the range
	 * [0..1) will darken the image while values above 1 will brighten the input
	 * image. This application also saves the processed
	 ...
	 * </blockquote></font></b>
	 * 
	 * @param args is an array of two strings. The first is the filename of the file
	 *             to process and the second is a perctage value to brighten/darken
	 *             an image.
	 * @throws IOException.
	 */
	public static void main(String[] args) throws IOException {
